...
export function SortableClip({ clip, isSelected, onSelect, onContext }: SortableClipProps) {
  ...
  return (
    <div
      ...
      className={`min-w-52 p-4 rounded-lg cursor-grab active:cursor-grabbing border ${
        isSelected
    ...
    >
      ...
      <div className="text-xs text-slate-400 mt-2">
        Timeline: {formatTime(clip.timelineStartTime)} -{" "}
      ...
      <div className="text-xs text-slate-500 mt-1">
        Length: {formatTime(clip.endTime - clip.startTime)} 
      </div>
    </div>
  );
}
